...
- OSC heartbeat (status out):
    * configurable IP/port
    * sends /show/status once per second with basic state
...
from pythonosc.udp_client import SimpleUDPClient  # OSC heartbeat client
...
class AudioPWMDualPlasmaApp:
    def __init__(self, root):
        ...
        # --- OSC heartbeat (status out) ---
        self.hb_enabled = tk.BooleanVar(value=False)
        self.hb_ip = tk.StringVar(value="127.0.0.1")
        self.hb_port = tk.IntVar(value=9100)
        self._hb_client = None
        self._hb_target = None

        # Let heartbeat client update when values change
        self.hb_enabled.trace_add("write", lambda *a: self._update_hb_client())
        self.hb_ip.trace_add("write",      lambda *a: self._update_hb_client())
        self.hb_port.trace_add("write",    lambda *a: self._update_hb_client())
    ...
    def _build_ui(self):
        ...
        osc = ttk.LabelFrame(t3, text="OSC Remote Control (listener)", padding=10)
        ...
        # --- OSC Heartbeat / status-out ---
        hb = ttk.LabelFrame(t3, text="OSC Heartbeat (status out)", padding=10)
        hb.pack(fill=tk.X, pady=(4, 4))

        ttk.Checkbutton(
            hb,
            text="Enable heartbeat (/show/status)",
            variable=self.hb_enabled,
            command=self._update_hb_client,
        ).grid(row=0, column=0, columnspan=4, sticky=tk.W, pady=(0, 6))

        ttk.Label(hb, text="Target IP:").grid(row=1, column=0, sticky=tk.W)
        ttk.Entry(hb, textvariable=self.hb_ip, width=16).grid(
            row=1, column=1, sticky=tk.W, padx=4
        )

        ttk.Label(hb, text="Target Port:").grid(row=1, column=2, sticky=tk.W)
        ttk.Entry(hb, textvariable=self.hb_port, width=8).grid(
            row=1, column=3, sticky=tk.W, padx=4
        )
    ...
    def _setup_autosave_traces(self):
        vars_to_trace = [
            ...
            self.hb_enabled, self.hb_ip, self.hb_port,
        ]
    ...
    def _settings_snapshot(self):
        return {
            ...
            "hb_enabled": bool(self.hb_enabled.get()),
            "hb_ip": self.hb_ip.get(),
            "hb_port": int(self.hb_port.get()),
        }
    ...
    def _load_settings(self):
        ...
        try:
            ...
            self.hb_enabled.set(bool(cfg.get("hb_enabled", False)))
            self.hb_ip.set(cfg.get("hb_ip", "127.0.0.1"))
            self.hb_port.set(int(cfg.get("hb_port", 9100)))
            self.status_var.set("Settings loaded from last session.")
            # Build heartbeat client from loaded settings
            self._update_hb_client()
        except Exception as e:
    ...
    # -------------------------- OSC heartbeat (status) -----------------------

    def _update_hb_client(self, *args):
        """Create / update the OSC heartbeat client when IP/port or enable flag changes."""
        if not self.hb_enabled.get():
            self._hb_client = None
            self._hb_target = None
            return

        ip = self.hb_ip.get().strip() or "127.0.0.1"
        try:
            port = int(self.hb_port.get())
        except Exception:
            port = 9100
            self.hb_port.set(port)

        try:
            self._hb_client = SimpleUDPClient(ip, port)
            self._hb_target = (ip, port)
            self.status_var.set(f"Heartbeat enabled → {ip}:{port}")
        except Exception as e:
            self._hb_client = None
            self._hb_target = None
            self.status_var.set(f"Heartbeat error: {e}")

    def _heartbeat_loop(self):
        """
        Send a small OSC status packet every 1s when enabled.

        /show/status  [is_playing, hardware_running, norm, duty, led_norm]
        """
        while True:
            time.sleep(1.0)
            try:
                if self._hb_client is not None and self.hb_enabled.get():
                    self._hb_client.send_message(
                        "/show/status",
                        [
                            1 if self.is_playing else 0,
                            1 if self.hardware_running else 0,
                            float(self.current_norm),
                            float(self.current_duty),
                            float(self.current_led_norm),
                        ],
                    )
            except Exception as e:
